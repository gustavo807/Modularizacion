@extends('vinculado.cuerpo')
@section('htmlheader_title')  @endsection
@section('contentheader_title') {{ $p->nombre }} @endsection
@section('contentheader_description')  @endsection

@section('main-content')
	<div class="container spark-screen">
		<div class="row">
			<div class="col-md-10 col-md-offset-1">
				<div class="panel panel-default">
					<div class="panel-heading">Investigadores</div>

					<div class="panel-body">
						<a href="/proyectos/{{{ $p->id }}}/edit" class="float"><i class="fa fa-plus my-float"></i></a>
						<table class="table table-bordered table-striped table-hover">
							<thead>
								<tr>
									<th>Nombre</th>
									<th>Email</th>
									<th>Grado</th>
									<th>Campo</th>
									<th>SNI</th>
								</tr>
							</thead>
							<tbody>
								@foreach($p->investigadores as $investigador)
									@if($investigador->sede_id == Auth::user()->sede->id)
										<tr>
											<td>
												{{ 
												   $investigador->nombre.' '.
												   $investigador->apellidopaterno.' '.
												   $investigador->apellidomaterno 
												}}
											</td>
											<td>{{ $investigador->correo }}</td>
											<td>{{ $investigador->grado }}</td>
											<td>{{ $investigador->campo }}</td>
											<td>{{ $investigador->sni }}</td>
										</tr>
									@endif
								@endforeach
							</tbody>								
				    	</table>
					</div>

				</div>
			</div>
		</div>
	</div>
@endsection